...
export interface TimelineBoardProps {
  ...
  extendViewport?: (opts: { addDaysStart?: number; addDaysEnd?: number }) => void | Promise<void>
}
...
const TimelineHeader = React.forwardRef<
  { monthsRef: HTMLDivElement | null; daysRef: HTMLDivElement | null },
  {
    viewport: TimelineViewport
    scrollLeft?: number
    clientWidth?: number
  }
>(({ viewport, scrollLeft = 0, clientWidth = 0 }, ref) => {
  const pxPerDay = viewport.pxPerDay ?? 16
  ...
  return (
    <div className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b border-gray-200">
      <div className="h-7 flex items-center text-xs text-gray-700">
        <div className="w-64 shrink-0 px-2">{/* left gutter for titles */}</div>
        <div className="flex-1 overflow-hidden">
          <div className="relative overflow-hidden pr-6">
            <div ref={monthsRef} style={{ transform: `translateX(-${scrollLeft}px)`, willChange: 'transform' }}>
              <div
                className="grid"
      ...
      <div className="h-6 flex items-center text-[11px] text-gray-500">
        ...
        <div className="flex-1 overflow-hidden">
          <div className="relative overflow-visible">
            <div ref={daysRef} style={{ transform: `translateX(-${scrollLeft}px)`, willChange: 'transform' }}>
...
export function TimelineBoard({
  ...
  const headerRef = useRef<{ monthsRef: HTMLDivElement | null; daysRef: HTMLDivElement | null }>(null)
  ...
  useEffect(() => {
    ...
    if (!sc) return
    const update = () => {
      setClientWidth(sc.clientWidth || 0)
      setScrollLeft(sc.scrollLeft || 0)
      prevScrollLeftRef.current = sc.scrollLeft || 0
    }
    update()
    const ro = new ResizeObserver(update)
    ro.observe(sc)
  ...
  // Center the initial view once when total width is known
  useEffect(() => {
    const sc = scrollerRef.current
    if (!sc || positionedRef.current) return
    const total = Math.max(sc.scrollWidth, 0)
    const target = Math.max(0, Math.floor(total / 2 - sc.clientWidth / 2))
    try {
      sc.scrollLeft = target
      setScrollLeft(target)
      prevScrollLeftRef.current = target
      positionedRef.current = true
    } catch {}
  }, [])

  // After outer viewport changes, apply any pending left-offset to preserve inner viewport
  useLayoutEffect(() => {
    const sc = scrollerRef.current
    if (!sc) return
    const pending = pendingScrollPxRef.current
    if (pending !== 0) {
      try {
        sc.scrollLeft = (sc.scrollLeft || 0) + pending
        setScrollLeft(sc.scrollLeft || 0)
        prevScrollLeftRef.current = sc.scrollLeft || 0
      } finally {
  ...
  const pxPerDay = viewport.pxPerDay ?? 16
  const totalPx = days * pxPerDay
  // Use an effective scrollLeft that includes any pending left-offset to avoid a one-frame day shift
  const effectiveScrollLeft = scrollLeft + pendingScrollPxRef.current
          ...
          <div className="flex-1 overflow-auto relative" ref={rightPaneRef as any}>
            <div
              ...
              onScroll={(e) => {
                ...
                // Update header transforms directly without waiting for React state
                if (headerRef.current?.monthsRef && headerRef.current?.daysRef) {
                  headerRef.current.monthsRef.style.transform = `translateX(-${sl}px)`
                  headerRef.current.daysRef.style.transform = `translateX(-${sl}px)`
                }
                ...
                const threshold = 120
                const stepDays = 60
                if (sl < threshold && extendViewport && dir < 0) {
                  const deltaPx = stepDays * (viewport.pxPerDay ?? 16)
                  // Mark pending scroll adjust to preserve visual position after left extension
                  pendingScrollPxRef.current += deltaPx
                  extendViewport({ addDaysStart: stepDays })
                } else if (sl > max - threshold && extendViewport && dir > 0) {
                  extendViewport({ addDaysEnd: stepDays })
                }
